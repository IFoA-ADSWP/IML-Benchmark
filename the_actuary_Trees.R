...
for (i in 1:CV){
  ...
  # GAM -------------------------------------------------
  # Train on same columns as GLM (no offset; drop IDpol, Exposure)
  models[[iter]]$GAM_model <- train_GAM(
    dt = dt_list$fre_mtpl2_freq[train_rows, -c(1,3)]
  )
  
  # Predict on held-out fold
  results[[iter]]$GAM <- predict_GAM(
    model   = models[[iter]]$GAM_model,
    newdata = dt_list$fre_mtpl2_freq[-train_rows, -c(1,3)],
    type    = "response"
  )
  
  # Loss
  losses$GAM[i] <- poisson_deviance(
    y_true = results[[iter]]$actual,
    y_pred = results[[iter]]$GAM
  )
  # XGBoost  -------------------------------------------
  ...
  results[[iter]]$XGB_init_GLM <- predict(
    models[[iter]]$XGB_init_GLM_model,
    dt_list$fre_mtpl2_freq[-train_rows, -c(1,2,3)]  # pass a data.frame; wrapper sets base_margin
  )
